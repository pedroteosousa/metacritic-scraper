...
const searchURL = '/search/{1}/{2}/results?page=0'
...
var search = function (text, options = {}) {
	...
	var results = []
	url = helper.format(helper.baseURL + searchURL, [options.category, text])
	helper.request(url)
		.on('end', (data) => {
			const $ = cheerio.load(data)
			$('.result').filter((h) => {
				var data = $($('.result')[h])
				var product = helper.createObject(data, searchInfo)
